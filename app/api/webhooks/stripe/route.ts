  ...
  try {
    ...
    switch (event.type) {
      case 'payment_intent.succeeded':
        ...
        if (paymentIntent.metadata?.phoneNumber) {
          await sendPaymentConfirmation(
            paymentIntent.metadata.phoneNumber,
            paymentIntent.metadata?.bookingId || paymentIntent.metadata?.tripId || 'N/A',
            paymentIntent.amount / 100, // Convert from cents/paise
      ...
      case 'checkout.session.completed':
        ...
        // Send WhatsApp payment confirmation
        if (session.metadata?.phoneNumber && session.amount_total) {
          await sendPaymentConfirmation(
            session.metadata.phoneNumber,
            session.metadata?.bookingId || session.metadata?.tripId || 'N/A',
            session.amount_total / 100, // Convert from cents/paise
