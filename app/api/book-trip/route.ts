...
export async function POST(req: NextRequest) {
  try {
    ...
    // Send WhatsApp confirmation if booking was successful and phone number is provided
    if (allSuccessful && processedUserDetails.phone) {
      try {
        // Determine transport type and details
        let transportType = 'Transport';
        let transportDetails = '';
        let fromCity = 'N/A';
        let toCity = 'N/A';
        let travelDate = 'N/A';

        if (confirmationDetails.flightBooking && tripPlan.transport) {
          const flight = tripPlan.transport as FlightData;
          transportType = 'Flight';
          transportDetails = `Flight: ${flight.flightNumber || 'N/A'}\nPNR: ${confirmationDetails.flightBooking.pnr}`;
          fromCity = flight.departureCity || flight.departureAirportIata;
          toCity = flight.arrivalCity || flight.arrivalAirportIata;
          travelDate = flight.departureDate || flight.departureTime.split('T')[0];
        } else if (confirmationDetails.trainBooking && tripPlan.transport) {
          const train = tripPlan.transport as TrainData;
          transportType = 'Train';
          transportDetails = `Train: ${train.trainNumber || 'N/A'}\nPNR: ${confirmationDetails.trainBooking.pnr}`;
          fromCity = train.departureCity;
          toCity = train.arrivalCity;
          travelDate = train.departureDate;
        } else if (confirmationDetails.busBooking && tripPlan.transport) {
          const bus = tripPlan.transport as BusData;
          transportType = 'Bus';
          transportDetails = `Bus: ${bus.operator || 'N/A'}\nPNR: ${confirmationDetails.busBooking.pnr}`;
          fromCity = bus.departureCity;
          toCity = bus.arrivalCity;
          travelDate = bus.departureDate;
        }

        await sendBookingConfirmation(
          processedUserDetails.phone,
          {
            bookingId,
            customerName: processedUserDetails.name,
            from: fromCity,
            to: toCity,
            date: travelDate,
            transportType,
            transportDetails,
            hotelName: tripPlan.hotel?.name,
            totalAmount,
        ...
        // Also make a voice call confirmation (optional - can be enabled via settings)
        const enableVoiceCall = process.env.ENABLE_VOICE_CALLS === 'true';
        if (enableVoiceCall) {
          try {
            ...
            if (callResult.success) {
              console.log('Voice call confirmation initiated:', callResult.callSid);
            }
          } catch (callError) {
            console.error('Failed to initiate voice call:', callError);
            // Don't fail the booking if voice call fails
          }
        }
      } catch (whatsappError) {
