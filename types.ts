...
export type TripPlanData = {
  ...
  cabOptionsToStation?: CabOption[];
  cabOptionsToHotel?: CabOption[];
  food?: string[];
  activities?: string[];
  groupSize?: number;
  total: number;
...
export type ParsedTripDetails = {
  intent: 'book_trip' | 'book_hotel' | 'book_car' | 'display_trip' | 'cancel_trip' | 'greet' | 'error' | 'unknown';
  message?: string;
  from: string | null;
  to: string | null;
  date: string | null;
  budget: 'Luxury' | 'Medium' | 'Budget-friendly' | null;
  mode: 'Train' | 'Bus' | 'Flight' | 'Car' | null;
